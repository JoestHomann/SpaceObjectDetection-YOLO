...
1.1 Object Classes in the Spark 2022 Stream 1 dataset List of Figures
2.1 Basic architecture of our model
2.2 Training data from all runs.
2.3 Validation data from all runs.
3.1 8x8 grid with center, class, and bounding box predictions
3.2 Confusion matrix showing the class accuracy
4.1 File-Stucture
4.2 Tensorboard training metrics of the last run
4.3 Tensorboard validation metrics of the last run
List of Tables
...
up convergence of the training. Before feeding an image into ResNet18, it is resized to 320x
...
LBCE(pi,j,yi,j) =− (yi,jlog(pi,j) + (1− yi,j) log(1− pi,j)). (2.3)
Here, the pi,jrepresents the calculated probability that the center point lies in the celli,i. This yi,j
...
Ltotal= wcLcenter + wbLbox + wclLclass (2.6)
where wc= 3, wb= 5, wcl= 1
2.3 Hyperparameter Optimization and Results
...
learning rate weight decay σ k-factor box weight
[ 1 e− 4 , 3e−4] [ 1 e− 4 , 1e−5] [0. 3 , 0. 5 ,0.7] [ 6 , 8 , 10] [ 3 , 5 , 8]
...
To improve on these issues, we suggest the following improvements.^1 These improvements are:
...
performance.
(^1) It should be noted that this list is not complete - there might be more possibilities to improve our model’s
performance.
